    ...
    private void buscarConductor() {
        try {
            ...
            if (pruebaActual != null) {
                double promedio = (pruebaActual.getNotaPercepcion() +
                        pruebaActual.getNotaAtencion() +
                        pruebaActual.getNotaCoordinacion() +
                        pruebaActual.getNotaPsicologica() +
                        pruebaActual.getNotaReaccion()) / 5.0;

                String estado = pruebaActual.estaAprobado() ? "APROBADO" : "REPROBADO";
