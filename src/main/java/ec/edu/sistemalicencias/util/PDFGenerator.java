...
public class PDFGenerator {
    ...
    private static void agregarDatosPrueba(Document documento, PruebaPsicometrica prueba)
        ...
        double promedioCalculado = (prueba.getNotaReaccion() +
                prueba.getNotaAtencion() +
                prueba.getNotaCoordinacion() +
                prueba.getNotaPercepcion() +
                prueba.getNotaPsicologica()) / 5.0;

        String estadoTexto = prueba.estaAprobado() ? "APROBADO" : "REPROBADO";
        ...
        PdfPCell celdaValor = new PdfPCell(new Phrase(
                String.format("%.2f", promedioCalculado), // Usamos la variable local
                FONT_CAMPO
        ));
        ...
        celdaValor = new PdfPCell(new Phrase(estadoTexto, fontEstado)); // Usamos la variable local
        celdaValor.setBackgroundColor(new BaseColor(240, 240, 240));
        celdaValor.setPadding(5);
        tabla.addCell(celdaValor);
